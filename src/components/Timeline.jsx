...
export default function JourneyTimeline({ checkpoints, title, subtitle }) {
  const [position, setPosition] = useState(checkpoints[0]?.position || 0);
  ...
  const handleMove = (clientX) => {
    ...
    const nearest = checkpoints.reduce((prev, curr) =>
      Math.abs(curr.position - percentage) <
  ...
  const handleTouchMove = (e) => e.touches && handleMove(e.touches[0].clientX);

  const handleEnd = () => {
    if (isDragging) {
      const nearest =
        checkpoints.find((c) => c.id === activeCheckpoint) || checkpoints[0];
      if (nearest) setPosition(nearest.position);
      setIsDragging(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="max-w-5xl mx-auto px-4 sm:px-8 flex items-center justify-between">
          <div className="text-left sm:text-center sm:flex-1">
            ...
            >
              Interactive timeline of your adventure
            </p>
          </div>
          ...
          <div className="sm:ml-4">
            <button
              ...
              className="cursor-pointer text-xl sm:text-3xl hover:scale-110 active:scale-95 transition-transform rounded-full p-2 shadow-lg"
              style={{
      ...
      {/* Details Card */}
      <div className="max-w-5xl mx-auto px-4 h-screen flex flex-col justify-center">
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-9">
            <div
              className={`rounded-2xl shadow-sm p-4 sm:p-8 h-full mb-8 sm:mb-12 transition-colors flex flex-col ${
                darkMode
          ...
          {/* Progress Stats */}
          <div className="md:col-span-3 xs:col-span-12">
            <div className="flex flex-col gap-8">
              <div
                className={`flex-1 rounded-xl p-10 text-center transition-colors ${
                  darkMode
                    ? "bg-slate-800 border border-slate-700"
                    : "bg-white border border-slate-200"
                }`}
      ...
      >
        <div className="max-w-5xl mx-auto" ref={pathRef}>
          {/* Road Container */}
          <div className="relative h-16 rounded-2xl overflow-visible px-2 flex items-center transition-colors bg-transparent">
            {/* Center line */}
            <div
              className={`absolute top-1/2 left-2 right-2 h-0.5 transform -translate-y-1/2 ${
            ...
            >
              ...
              >
                ...
                >
                  <span style={{ fontSize: "40px" }} className="sm:text-4xl">
                    🏃🏻
                  </span>
                </div>
